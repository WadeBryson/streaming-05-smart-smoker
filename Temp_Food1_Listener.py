...
def Food1_callback(ch, method, properties, body):
    ...
    try:
        ...
        food1_info = body.decode().split(",")
        # Making sure the temperature is valid before we add to the deque
        if food1_info[1] != "No Temperature":
            food1_temperature = float(food1_info[1])
            food1_timestamp = food1_info[0]
            Food1_Deque.append(food1_temperature)

            # Temperature Alert Calculations
            if len(Food1_Deque) >= 20:
                temperature_difference = Food1_Deque[-1]-Food1_Deque[0]
                if temperature_difference < 1:
                    alert_needed = True

                    if alert_needed:
                        logger.info("Food1 Alert: At {food1_timestamp} your food temperature has stalled for 10 minutes.")
...
def main(hn: str = "localhost", qn: str = "Temp_Food1_Queue"):
...
if __name__ == "__main__":
    ...
    main("localhost", "Temp_Food1_Queue")